...
pub const NativeAPI = struct {
    ...
    pub fn registerClass(gd: NativeAPI, name: [*:0]const u8, base: [*:0]const u8, constructor: ConstructorFn, destructor: DestructorFn) Class {
        ...
        return Class{ .name = name, .base = base };
    }

    pub fn registerMethod(gd: NativeAPI, class: Class, method_name: [*:0]const u8, method: MethodFn) void {
        const attributes: c.godot_method_attributes = .{ .rpc_type = c.GODOT_METHOD_RPC_MODE_DISABLED };
        c.init_class_method(gd.nativescript_api, gd.p_handle, class.name, method_name, attributes, method, null, null);
    }
};
